...
HI_S32 PcivFmw_ResetCallBack(HI_S32 s32DevId, HI_S32 s32ChnId, HI_VOID *pvData)
{
	...
	while(1)
	{
		...
		else
		{
			...
			{
				HI_ASSERT(pChn->pCurVdecNode != NULL);
				s32Ret = VB_UserSub(pChn->pCurVdecNode->stPcivPic.stVideoFrame.u32PoolId, pChn->pCurVdecNode->stPcivPic.stVideoFrame.stVFrame.u32PhyAddr[0], VB_UID_PCIV);
		        HI_ASSERT(s32Ret == HI_SUCCESS);
	...
	for (i=0; i<u32BusyNum; i++)
	{
		...
		s32Ret = VB_UserSub(pNode->stPcivPic.stVideoFrame.u32PoolId, pNode->stPcivPic.stVideoFrame.stVFrame.u32PhyAddr[0], VB_UID_PCIV);
        HI_ASSERT(s32Ret == HI_SUCCESS);
