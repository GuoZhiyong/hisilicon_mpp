...
static HI_U32 HashMsgPadding(HI_U8 *pu8Msg, HI_U32 u32ByteLen)
{
    ...
    memset(&pu8Msg[u32Total], 0, u32PaddingLen - 1 - 8);
...
HI_S32 SHA1_2()
{
    ...
    for(i = 0; i < 5; i++)
    {
        if(i == 3)
        {
            memset(pInputAddrVir, 'a', LONG_DATA_SIZE);
...
//        printBuffer("hmac-Sha1 result:", u8Hash, 20);
        ...
        printf("HMAC SHA1 run success, sample %d!\n", i);
    }
...
HI_S32 HMAC_SHA256()
{
 ...
 //       printBuffer("hmac-Sha256 result:", u8Hash, 32);
        ...
        printf("HMAC SHA256  run success, sample %d!\n", i);
    }
