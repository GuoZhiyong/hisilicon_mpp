...
#else
    unsigned int mipi_reg = IO_ADDRESS(0x20670004);
#endif
...
#else
    unsigned int sensor_reg = IO_ADDRESS(0x20670000);
...
void  hi_mipi_init(void)
{
    ...
    mipi_write_reg32(mipi_apb_reset, reset << 11, 1 << 11);
#else
    unsigned int mipi_apb_reset = IO_ADDRESS(0x20670008);
...
static inline void mipi_int_statics(COMBO_LINK link_id)
{
    ...
    if (int_raw || int2_raw)
    {
        ...
        *(volatile HI_U32*)IO_ADDRESS( 0x20681214) = 0x1100;
        ...
        /* MIPI RESET */
        *(volatile HI_U32*)IO_ADDRESS(0x20670004)  = 1;
        mipi_drv_set_csi2_rstn(0, 0);
        mipi_drv_set_csi2_rstn(0, 1);

        /* MIPI UNRESET */
        *(volatile HI_U32*)IO_ADDRESS(0x20670004)  = 0;
        *(volatile HI_U32*)IO_ADDRESS(0x20681214)  = 0x1100;
#endif
    }
